...
export const $toaster = deepMap<{ [key: string]: ToasterItemProps }>({
  default: {},
});
...
class ToastService {
  ...
  async promise(
    fn: () => Promise<any>,
    props: ToastPromiseProps<Omit<ToastProps, "noDissapear">>,
  ) {
    const toasterId = props.toasterId || "default";
  ...
  success(
    msg: string,
    props?: Omit<ToastRenderProps<ToastProps>, "description">,
  ) {
    return this.createToast({
      ...props,
      description: msg,
